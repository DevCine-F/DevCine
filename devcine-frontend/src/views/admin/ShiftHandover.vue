...
const loadData = async () => {
  ...
  try {
    ...
    const requests = [shiftStore.fetchCurrentSummary().catch(() => null)]
    if (canViewList.value) requests.push(shiftHandoverApi.list())
    const [summaryData, listRes] = await Promise.all(requests)
...
const handleSubmit = async () => {
  ...
  const ok = await confirm.show({
    ...
    message: 'Sau khi gửi, quản lý sẽ đối soát và xác nhận biên bản này.',
...
const handleDecision = async (handover, action) => {
  const ok = await confirm.show({
    title: action === 'confirm' ? 'Xác nhận bàn giao' : 'Từ chối bàn giao',
    message: `Biên bản #${handover.id} sẽ được cập nhật trạng thái.`,
    confirmText: action === 'confirm' ? 'Xác nhận' : 'Từ chối',
  ...
  try {
    ...
    toast.success(action === 'confirm' ? 'Đã xác nhận bàn giao.' : 'Đã từ chối bàn giao.')
    await loadData()
  } catch (error) {
  ...
  <div class="p-8 space-y-6">
    <header class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <p class="text-sm text-on-surface-variant">Đối soát doanh thu, số vé và tiền mặt theo ca làm việc.</p>
      </div>
    ...
    <section v-else-if="hasSummary" class="space-y-5">
      ...
      <div class="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
        <div class="rounded-lg border border-outline-variant/10 bg-surface p-6">
          <h2 class="text-lg font-black text-on-surface">Ca hiện tại</h2>
        ...
        <form class="rounded-lg border border-outline-variant/10 bg-surface p-6 space-y-4" @submit.prevent="handleSubmit">
          ...
          <label class="block">
            <span class="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Ghi chú</span>
          ...
          <AppButton class="w-full" :loading="isSubmitting">Gửi bàn giao</AppButton>
        </form>
      </div>
    </section>
    ...
    <section v-else class="rounded-lg border border-dashed border-outline-variant/20 bg-surface p-10 text-center">
      ...
      <p class="mt-3 text-lg font-black text-on-surface">Chưa có ca đang hoạt động</p>
      <p class="mt-1 text-sm text-on-surface-variant">Bạn cần có ca đã duyệt, đúng giờ để bàn giao cuối ca.</p>
    </section>

    <section v-if="canViewList" class="rounded-lg border border-outline-variant/10 bg-surface overflow-hidden">
      <div class="border-b border-outline-variant/10 p-5">
        <h2 class="text-lg font-black text-on-surface">Biên bản gần đây</h2>
      </div>
          ...
          <tbody class="divide-y divide-outline-variant/10">
            <tr v-for="handover in handovers" :key="handover.id">
              ...
              <td class="px-4 py-3 text-center">
                <span class="rounded-full px-3 py-1 text-[10px] font-black uppercase" :class="handover.status === 'CONFIRMED' ? 'bg-green-500/10 text-green-400' : handover.status === 'REJECTED' ? 'bg-red-500/10 text-red-300' : 'bg-primary/10 text-primary'">{{ handover.status }}</span>
              </td>
              <td class="px-4 py-3 text-right">
                <div v-if="canManage && handover.status === 'PENDING'" class="flex justify-end gap-2">
                  <AppButton size="sm" variant="outline" @click="handleDecision(handover, 'reject')">Từ chối</AppButton>
                  <AppButton size="sm" @click="handleDecision(handover, 'confirm')">Xác nhận</AppButton>
                </div>
              </td>
            </tr>
          </tbody>
