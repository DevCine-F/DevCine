...
const namePlaceholder = computed(() => {
  switch (form.value.type) {
    case 'COMBO': return 'VD: Combo Couple (1 Bắp + 2 Nước)'
    case 'POPCORN': return 'VD: Bắp Rang Bơ (Lớn)'
    case 'DRINK': return 'VD: Coca-Cola Tươi 32oz'
    case 'SNACK': return 'VD: Khoai tây lắc phô mai'
    default: return 'VD: Tên món ăn / thức uống'
  }
})

const descPlaceholder = computed(() => {
  switch (form.value.type) {
    case 'COMBO': return 'VD: 1 bắp lớn + 2 nước ngọt'
    case 'POPCORN': return 'VD: Bắp rang bơ thơm giòn vị truyền thống cỡ lớn'
    case 'DRINK': return 'VD: Nước ngọt có ga ly lớn 32oz mát lạnh'
    case 'SNACK': return 'VD: Đồ ăn vặt giòn ngon'
    default: return 'VD: Mô tả ngắn gọn về món'
  }
})

const autoGenerateDescription = () => {
  if (form.value.type !== 'COMBO') return
  if (!form.value.slots || form.value.slots.length === 0) {
    form.value.description = 'Combo cố định (không tùy chọn vị)'
    return
  }
  const counts = {}
  for (const s of form.value.slots) {
    const label = s.slotLabel?.trim() || poolById(s.optionGroupId)?.name || 'Món'
    const baseName = label.replace(/\s+\d+$/, '').trim()
    counts[baseName] = (counts[baseName] || 0) + 1
  }
  const parts = Object.entries(counts).map(([name, count]) => `${count} ${name}`)
  form.value.description = parts.join(' + ')
}
...
const openEdit = (item) => {
  ...
  // Chế độ Builder số lượng trực quan, an toàn
  builderMode.value = true
  resetTouched()
  isDrawerOpen.value = true
}
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              ...
              <button
                v-if="form.imageUrl && !isUploading"
                type="button"
                @click="form.imageUrl = ''"
                class="text-[10px] font-bold text-red-400 hover:text-red-300 transition-colors flex items-center gap-0.5"
              >
                <span class="material-symbols-outlined text-xs">delete</span> Xóa ảnh
              </button>
            </div>
            <div class="w-full h-36 bg-surface-container-highest border-2 border-dashed border-outline-variant/20 rounded-xl flex items-center justify-center overflow-hidden relative group">
          ...
          <p v-else-if="form.type === 'COMBO' && pricePreview.hasRange" class="text-[10px] text-primary/80 font-medium -mt-2">
            Tạm tính gồm phụ thu: {{ formatPrice(pricePreview.min) }} ~ {{ formatPrice(pricePreview.max) }}
          </p>
          ...
          <div class="space-y-1.5">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <button
                  v-if="form.type === 'COMBO' && form.slots.length > 0"
                  type="button"
                  @click="autoGenerateDescription"
                  class="text-[10px] font-bold text-primary hover:brightness-110 transition-all flex items-center gap-0.5"
                  title="Tự động tạo nội dung mô tả từ danh sách thành phần combo"
                >
                  <span class="material-symbols-outlined text-xs">auto_awesome</span> Tự động tạo
                </button>
                <span class="text-[10px] font-semibold tabular-nums text-on-surface-variant/60">{{ (form.description || '').length }}/255</span>
              </div>
            </div>
            <textarea v-model="form.description" rows="3" maxlength="255" class="w-full bg-surface-container-highest border border-outline-variant/20 p-3 rounded-lg text-sm text-on-surface focus:border-primary outline-none resize-none" :placeholder="descPlaceholder"></textarea>
          </div>
          ...
          <div v-if="form.type === 'COMBO'" class="p-4 rounded-xl border border-primary/20 bg-primary/5 space-y-1">
            <div class="flex items-center justify-between">
              <p class="text-[10px] font-bold uppercase tracking-widest text-primary">Giá khách trả (tạm tính)</p>
              <span v-if="pricePreview.hasRange" class="text-[9px] font-bold px-1.5 py-0.5 rounded-sm bg-amber-500/10 text-amber-400 border border-amber-500/20">
                Có phụ thu
              </span>
            </div>
            <p class="text-sm font-black text-on-surface">
              ...
              <template v-else>{{ formatPrice(pricePreview.min) }}</template>
            </p>
            <p v-if="pricePreview.hasRange" class="text-[10px] text-on-surface-variant">Đã bao gồm mức phụ thu tương ứng với các vị khách có thể chọn.</p>
          </div>
