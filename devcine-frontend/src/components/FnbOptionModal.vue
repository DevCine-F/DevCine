...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-surface-container rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
      ...
      <!-- Body -->
      <div class="p-6 overflow-y-auto space-y-4 flex-grow custom-scrollbar bg-surface">
        ...
        <template v-else-if="!activeSlotId">
          <div
            ...
            class="flex items-center justify-between p-4 bg-surface-container-highest rounded-xl cursor-pointer hover:bg-surface-container-highest/80 border border-outline-variant/20 transition-colors"
          >
            <div class="flex flex-col">
              <span class="font-bold text-sm text-on-surface">{{ slot.slotLabel }} <span v-if="slot.isRequired" class="text-red-400">*</span></span>
        ...
        <template v-else>
          <div class="flex justify-between items-end mb-2">
            <h4 class="font-bold text-on-surface text-sm text-on-surface-variant">{{ slotHint(activeSlot) }}</h4>
          </div>
