...
public class CustomerController {
    ...
    public ResponseEntity<?> listCustomers(@RequestParam(required = false) String q) {
        ...
        if (!customerIds.isEmpty()) {
            ...
            for (Object[] row : concessionSaleRepository.aggregateConcessionSpentAndCountByCustomerIds(customerIds)) {
                ...
                BigDecimal spent = (BigDecimal) row[1];
                Long cnt = ((Number) row[2]).longValue();
                spentMap.put(cid, spentMap.getOrDefault(cid, BigDecimal.ZERO).add(spent != null ? spent : BigDecimal.ZERO));
                countMap.put(cid, countMap.getOrDefault(cid, 0L) + (cnt != null ? cnt : 0L));
            }
        }
    ...
    public ResponseEntity<?> getCustomerOrders(@PathVariable Integer id) {
        ...
        if (!bookings.isEmpty()) {
            ...
            for (Booking b : bookings) {
                ...
                m.put("seats", String.join(", ", seatList));
    ...
    public ResponseEntity<?> getCustomerVouchers(@PathVariable Integer id) {
        ...
        List<Map<String, Object>> result = vouchers.stream().map(v -> {
            Map<String, Object> m = new HashMap<>();
            m.put("id", v.getId());
            m.put("code", v.getPromotion() != null ? v.getPromotion().getCode() : "VOUCHER");
