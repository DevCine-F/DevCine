...
> - **Chuẩn hóa nghiệp vụ Quản lý & Chỉnh sửa Suất chiếu** (31/08): Chặn chỉnh sửa suất quá khứ/đang diễn ra (`showtime.startTime < NOW()`); nghiêm cấm đổi phòng/phim/định dạng/giờ khi đã có vé bán (`reserved > 0`); cho phép cập nhật đầy đủ phim/định dạng/giờ và tự động tính lại `endTime` + re-snapshot layout khi chưa có vé bán; UI khóa tập trung từ Drawer chi tiết (`ShowtimeDetailsDrawer.vue`) và đồng bộ dropdown giờ/phút không bị mất dữ liệu (`ShowtimeDrawer.vue`).
...
> - **Bảo Vệ Quyền Sở Hữu Voucher Cá Nhân Khi Đổi Điểm (02/09):** Sửa lỗ hổng nghiêm trọng khiến khách mất điểm tích lũy oan khi đổi điểm lấy voucher rồi dùng chậm. Hai loại voucher nay có vòng đời quota độc lập: **(1) Voucher Công Khai (FCFS)** — quota tính khi thanh toán, giữ nguyên; **(2) Voucher Cá Nhân (Đổi điểm)** — quota được giữ chỗ atomic ngay lúc đổi điểm (`redeemWithPoints` gọi `incrementIssuedCountIfQuotaAvailable` TRƯỚC `loyaltyService.redeem`); nếu hết suất ném lỗi ngay, điểm không bị trừ. Voucher đã vào ví cá nhân miễn nhiễm check `usedCount >= usageLimit` trong `evaluate()` và `getOrClaimForCheckout()` (`isOwnedPersonally` flag). `BookingService.completePayment()` phân nhánh theo `getAllowPointRedemption()` — không tăng `usedCount` lần 2 cho voucher đổi điểm. `VoucherController` không gán `EXHAUSTED` cho voucher đổi điểm (`!isPointRedemption`). Frontend: label "Hết suất đổi" / "Hết lượt lưu" phân biệt 2 loại. **5 file sửa, ERD không đổi.**
> - **Đồng bộ Nhả ghế WebSocket DB-Aware & Khóa vững khi Thanh toán VNPAY / Chọn Voucher (03/09):** Khắc phục triệt để lỗi nhả ghế giả trên màn hình POS khi khách hàng chuyển sang trang thanh toán VNPAY hoặc click chọn Voucher/thêm Combo F&B: (1) `SeatLockService.releaseSession` tự động kiểm tra `bookingSeatRepository.findConflictingSeats`, chỉ gửi broadcast `SEAT_RELEASED` cho các ghế click tạm ở Bước 1, bảo vệ nguyên vẹn trạng thái `HOLD` cho ghế đã tạo đơn DB khi WebSocket bị ngắt kết nối do redirect VNPAY; (2) `PosHoldService.releaseHold(bookingId, broadcastRelease)` hỗ trợ cờ tắt phát broadcast khi được gọi từ `BookingService.holdSeats` trong luồng re-hold cập nhật đơn (Voucher / F&B), triệt tiêu hoàn toàn sự kiện nhả ghế giả gửi tới POS; (3) `SeatHoldCleanupScheduler` tự động gom nhóm và broadcast `SEAT_RELEASED` qua WebSocket khi các đơn `HOLD` quá hạn 10 phút, giúp POS tự động mở lại ghế trống (`AVAILABLE`) đúng giờ mà không cần tải lại trang; (4) Tại `BookingView.vue` và `TicketingPOS.vue`, khi khách/thu ngân bấm "Đổi suất khác", "Quay lại" ở bước chọn ghế hoặc chuyển trang/hủy đơn, hệ thống chủ động gọi `deselect`/`releaseHold` giải phóng toàn bộ ghế đang chọn, đưa ghế về trạng thái `AVAILABLE` cho các bên khác tức thì trong 0.1s.
