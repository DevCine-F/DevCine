# -*- coding: utf-8 -*-
"""
Script to generate Dac_Ta_Use_Case.docx containing all 72 Use Case Specifications.
"""
import sys
import docx
...
def add_use_case_table(doc, uc_num, uc_id, uc_name, priority, actor, description, steps, notes):
    ...
    # Column widths in dxa: ~1.5 inch, 2.5 inch, 1.5 inch, 2.5 inch (Total ~7.0 inch)
    col_widths = [Inches(1.4), Inches(2.1), Inches(1.4), Inches(2.1)]
    ...
    cell_31.text = ""
    p_step = cell_31.paragraphs[0]
    for idx, step in enumerate(steps):
        if idx > 0:
            p_step = cell_31.add_paragraph()
    ...
    cell_41.text = ""
    p_note = cell_41.paragraphs[0]
    for idx, note in enumerate(notes):
        if idx > 0:
            p_note = cell_41.add_paragraph()
